...
class Controle:
    def __init__(self, largura, n_inicial, n_final):
        try:
            # Manter a largura em metros (não converter para nanômetros)
            ...
            if self.n_inicial <= 0 or self.n_final <= 0:
             ...
             # Instanciar a classe Proton
            proton = Proton(self.largura, self.n_inicial, self.n_final)

             # Armazenar a instância de Proton no atributo self.proton
            self.proton = Proton(self.largura, self.n_inicial, self.n_final)
            ...
            print(f"Velocidade da particula no nível {self.n_inicial}: {proton.velocidade(self.n_inicial)} m/s")
            print(f"Velocidade da particula no nível {self.n_final}: {proton.velocidade(self.n_final)} m/s")
            print("-" * 100)

            print(f"Comprimento de Onda de De Broglie da particula no nível {self.n_inicial}: {proton.comprimento_de_broglie(self.n_inicial)} m ")
            print(f"Comprimento de Onda de De Broglie da particula no nível {self.n_final}: {proton.comprimento_de_broglie(self.n_final)} m ")
            print("-" * 100)

        except ValueError as ve:
            print(f"Erro: {ve}")

    def calcular_probabilidade(self, ponto1, ponto2, n):
        """Calcula a probabilidade entre dois pontos em um determinado nível"""
        probabilidade = self.proton.calcular_probabilidade(ponto1, ponto2, n)
